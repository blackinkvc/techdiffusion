...
(function () {
  ...
  function renderWorldview() {
    ...
    $$("#legend .chip").forEach(ch => ch.addEventListener("click", () => {
      state.cat = ch.dataset.cat; switchView("browse");
    }));
    // 注意：switchView 已集成历史写入
  }
  ...
  function renderFilterBar() {
    ...
    $$('#filter-bar [data-cat]').forEach(b => b.addEventListener("click", () => { state.cat = b.dataset.cat; renderBrowse(); renderFilterBar(); history.replaceState(encodeState(), "", "?" + stateToQuery(encodeState())); }));
    $$('#filter-bar [data-era]').forEach(b => b.addEventListener("click", () => { state.era = b.dataset.era; renderBrowse(); renderFilterBar(); history.replaceState(encodeState(), "", "?" + stateToQuery(encodeState())); }));
  }
  ...
  function renderTreeToolbar() {
    ...
    $$('#tree-toolbar [data-cat]').forEach(b => b.addEventListener("click", () => { state.cat = b.dataset.cat; renderTree(); history.replaceState(encodeState(), "", "?" + stateToQuery(encodeState())); }));
  ...
  function renderTree() {
    ...
    const links = [];
    TECHS.forEach(t => t._up.forEach(u => { if (ids.has(t.id) && ids.has(u)) links.push({ source: t.id, target: u }); }));

    const g = svg.append("g");
    const link = g.append("g").attr("stroke-width", 1.2).selectAll("line").data(links).join("line").attr("class", "link");
  ...
  function highlight(id) {
    ...
    // 边定义：{source: 依赖者, target: 前置技术} ⇒ source 依赖 target
    // 上游边：target===id（source 为上游节点，需在 t._up 中）
    // 下游边：source===id（target 为下游节点，需在 t._down 中）
    tree.link
  ...
  function renderTreeSide(t) {
    ...
    if (t._down.length === 0 && t.enables && t.enables.length) {
      // 全部为概念影响
    }
    if (t._down.length) {
      html += `<div class="m-label" style="margin-top:12px">↓ 催生的后续技术（下游）</div><div>`;
      t._down.forEach(d => html += `<span class="node-link" data-id="${d}">${esc(techMap[d].name)}<span class="nl-date">${esc(techMap[d].date)}</span></span>`);
      html += `</div>`;
    }
  ...
  function computeLineage(rootId) {
    ...
    TECHS.forEach(t => (t._up || []).forEach(u => {
      if (idSet.has(t.id) && idSet.has(u)) links.push({ source: t.id, target: u, kind: upGen[u] !== undefined ? "up" : "down" });
    }));
  ...
  function renderLineage(id) {
    ...
    node.on("click", (ev, d) => { ev.stopPropagation(); lineageState.id = d.id; const sel = $("#lineage-select"); if (sel) sel.value = d.id; const s = encodeState(); s.v = "lineage"; s.id = d.id; navigate(s); });
  ...
  function renderLineageSide(root, L) {
    ...
    $$("#lineage-side .lg-item").forEach(el => el.addEventListener("click", () => {
      lineageState.id = el.dataset.id; const sel = $("#lineage-select"); if (sel) sel.value = el.dataset.id;
      const s = encodeState(); s.v = "lineage"; s.id = el.dataset.id; navigate(s);
    }));
  }
  ...
  //  详情弹窗
  // ============================================================
  // 纯渲染：把详情写入弹窗，不触碰历史（供 applyState / popstate 复用，避免递归）
  function renderModalBody(id, fromTree) {
    ...
    let html = `
      <div class="modal-head">
        ...
        <div class="m-badges">
          ...
          <span class="badge" style="font-size:12px;padding:3px 10px;border-radius:12px;background:#eef2f7;color:#475569">${esc(t.date)}</span>
          ${t.place ? `<span class="badge" style="font-size:12px;padding:3px 10px;border-radius:12px;background:#eef2f7;color:#475569">${esc(t.place)}</span>` : ""}
        </div>
      </div>
    ...
    // 下游后续
    html += `<div class="m-row"><div class="m-label">↓ 催生的后续技术（本技术使之可能）</div><div class="m-tags">`;
    if (t._down.length) t._down.forEach(d => html += `<span class="m-tag linkable" data-id="${d}">${esc(techMap[d].name)}</span>`);
    else html += `<span class="m-tag">（尚未催生已收录的新技术）</span>`;
    if (t._downConcept.length) t._downConcept.forEach(c => html += `<span class="m-tag concept">${esc(humanize(c))}</span>`);
    html += `</div></div>`;
    ...
    if (t.significance) {
      html += `<div class="m-row"><div class="m-label">历史意义</div><div class="m-summary" style="background:#f0f7ff">${esc(t.significance)}</div></div>`;
    }
    ...
    $$("#modal-card [data-lineage]").forEach(b => b.addEventListener("click", () => {
      closeModal();
      lineageState.id = id;
      const s = encodeState(); s.v = "lineage"; s.id = id; s.modal = null; s.fromTree = false;
      navigate(s);
    }));

    $("#modal").classList.remove("hidden");
    if (fromTree) highlight(id);
  }
  // 历史封装：点击技术卡片 / 节点时调用，写入一条历史记录
  function openTech(id, fromTree) {
    const t = techMap[id]; if (!t) return;
    modalTechId = id; modalFromTree = !!fromTree;
    const s = encodeState(); s.modal = id; s.fromTree = !!fromTree;
    navigate(s);
  }
  function closeModal() {
    if (modalTechId) {
      // 关闭弹窗写入历史：回到当前视图（无弹窗）
      modalTechId = null; modalFromTree = false;
      const s = encodeState(); s.modal = null; s.fromTree = false;
      navigate(s);
    }
  ...
  function progTier(t) {
    ...
    if (!hasPP && (BANNED.test(sum) || sum.length < 40)) return "stub"; // 占位：无归属且套话/过短
    if (sum.length >= 80) return "good";                                // 优秀·深度撰写
    return "ok";                                                       // 合格·偏薄
  }
  function mixedLang(s) { s = s || ""; return /\p{Script=Han}+[与和]\s*[A-Za-z]/u.test(s); } // 仅禁止「中文+与/和+英文」式不良混排
  ...
  function renderImpGraph(wrap) {
    ...
    node.append("circle")
      .attr("r", d => 6 + (d._net.comp / maxC) * 22)
    ...
    const sim = d3.forceSimulation(nodes)
      ...
      .force("center", d3.forceCenter(W / 2, H / 2))
      .force("collide", d3.forceCollide(d => 8 + (d._net.comp / maxC) * 24))
  ...
  //  历史记录（浏览器前进 / 后退）
  // ============================================================
  // 单一可序列化状态：视图 + 各视图子状态 + 可选详情弹窗
  function encodeState() {
    const s = { v: state.view, cat: state.cat || "all", era: state.era || "all" };
    if (state.view === "browse" || state.view === "tree") {
      if (s.cat !== "all") s.cat = s.cat; // 保留过滤
      if (s.era !== "all") s.era = s.era;
    }
    if (state.view === "browse" && state.search.trim()) s.q = state.search.trim();
    if (state.view === "timeline") {
      if (timelineState.preset !== "all") s.preset = timelineState.preset;
      if (timelineState.preset === "custom") { s.s = timelineState.start; s.e = timelineState.end; }
    }
    if (state.view === "lineage") s.id = lineageState.id;
    if (modalTechId) { s.m = modalTechId; if (modalFromTree) s.mt = 1; }
    return s;
  }
  function decodeState(q) {
    const s = { v: "worldview", cat: "all", era: "all", search: "", preset: "all", start: -Infinity, end: Infinity, id: "smart_mower", modal: null, fromTree: false };
    if (!q) return s;
    if (q.v) s.v = q.v;
    if (q.cat) s.cat = q.cat; else s.cat = "all";
    if (q.era) s.era = q.era; else s.era = "all";
    if (q.q) s.search = q.q;
    if (q.preset) s.preset = q.preset; else s.preset = "all";
    if (s.preset === "custom") { s.start = q.s !== undefined ? +q.s : -Infinity; s.end = q.e !== undefined ? +q.e : Infinity; }
    if (q.id) s.id = q.id;
    if (q.m) { s.modal = q.m; s.fromTree = !!q.mt; }
    return s;
  }
  function stateToQuery(s) {
    const p = new URLSearchParams();
    p.set("view", s.v || "worldview");
    if (s.cat && s.cat !== "all") p.set("cat", s.cat);
    if (s.era && s.era !== "all") p.set("era", s.era);
    if (s.v === "browse" && s.search) p.set("q", s.search);
    if (s.v === "timeline") {
      p.set("preset", s.preset || "all");
      if (s.preset === "custom") { p.set("s", s.start); p.set("e", s.end); }
    }
    if (s.v === "lineage" && s.id) p.set("id", s.id);
    if (s.modal) { p.set("modal", s.modal); if (s.fromTree) p.set("mt", "1"); }
    return p.toString();
  }
  function queryToState(search) {
    const p = new URLSearchParams(search.replace(/^\?/, ""));
    const q = {};
    if (p.get("view")) q.v = p.get("view");
    if (p.get("cat")) q.cat = p.get("cat");
    if (p.get("era")) q.era = p.get("era");
    if (p.get("q")) q.q = p.get("q");
    if (p.get("preset")) q.preset = p.get("preset");
    if (p.get("s") !== null) q.s = p.get("s");
    if (p.get("e") !== null) q.e = p.get("e");
    if (p.get("id")) q.id = p.get("id");
    if (p.get("modal")) q.m = p.get("modal");
    if (p.get("mt")) q.mt = 1;
    return decodeState(q);
  }

  let modalTechId = null, modalFromTree = false;
  let restoring = false; // 防止 popstate 与 pushState 互相触发

  // 应用一个状态对象到界面（不修改历史）
  function applyState(s, opts) {
    opts = opts || {};
    state.view = s.v;
    state.cat = s.cat; state.era = s.era; state.search = s.search || "";
    if (s.v === "timeline") { timelineState.preset = s.preset; timelineState.start = s.start; timelineState.end = s.end; }
    if (s.v === "lineage" && s.id) lineageState.id = s.id;

    // 全局搜索框
    const gs = $("#globalSearch"); if (gs && state.search) gs.value = state.search;

    // 视图切换（仅渲染目标视图）
    $$("#nav-main a").forEach(t => t.classList.toggle("active", t.dataset.view === s.v));
    $$(".view").forEach(v => v.classList.toggle("active", v.id === "view-" + s.v));
    if (s.v === "browse") renderBrowse();
    else if (s.v === "timeline") { syncSliders(); renderTimeline(); }
    else if (s.v === "tree") renderTree();
    else if (s.v === "lineage") { const sel = $("#lineage-select"); if (sel) sel.value = lineageState.id; renderLineage(lineageState.id); }
    else if (s.v === "worldview") renderWorldview();
    else if (s.v === "method") renderMethod();
    else if (s.v === "midtech") renderMidtech();
    else if (s.v === "progress") renderProgress();
    else if (s.v === "sop") renderSop();
    else if (s.v === "analysis") renderAnalysis();

    // 详情弹窗（直接渲染，不二次 push 历史）
    if (s.modal && techMap[s.modal]) {
      modalTechId = s.modal; modalFromTree = s.fromTree;
      renderModalBody(s.modal, s.fromTree);
    } else {
      modalTechId = null; modalFromTree = false;
      $("#modal").classList.add("hidden");
    }
  ...
  // 导航到新状态：渲染并 push 进历史
  function navigate(s, replace) {
    applyState(s);
    const url = "?" + stateToQuery(encodeState());
    if (replace) history.replaceState(encodeState(), "", url);
    else {
      history.pushState(encodeState(), "", url);
      // 维护历史栈深度（用于后退按钮可用性）
      histDepth++;
      window.__canBack = histDepth > 0;
      window.__canFwd = false;
    }
    updateHistButtons();
  }
  ...
  function switchView(v) {
    const s = encodeState();
    s.v = v;
    // 切换视图时关闭弹窗（除非是同视图内子状态变化）
    s.modal = null; s.fromTree = false; modalTechId = null; modalFromTree = false;
    navigate(s);
  }

  function updateHistButtons() {
    const back = $("#hist-back"), fwd = $("#hist-fwd");
    if (back) back.disabled = !(window.__canBack);
    if (fwd) fwd.disabled = !(window.__canFwd);
  }
  // 维护前进/后退可用性标记
  let histDepth = 0; // 本站内 pushState 的次数（首屏 replaceState 不计数）
  window.__canBack = false; window.__canFwd = false;
  ...
  function init() {
    ...
    // 时间轴尺度：确定时间下界
    TL_MIN = Math.min(...TECHS.map(t => t.year));
    renderTimelineControls();

    // 来龙去脉：技术选择器
    const sel = $("#lineage-select");
    ...
    sel.addEventListener("change", () => { lineageState.id = sel.value; const s = encodeState(); s.v = "lineage"; s.id = sel.value; navigate(s); });
    $$("#nav-main a").forEach(t => t.addEventListener("click", e => { e.preventDefault(); switchView(t.dataset.view); }));
    $("#globalSearch").addEventListener("input", e => {
      state.search = e.target.value;
      if (state.view === "browse") renderBrowse();
      else if (state.view === "timeline") renderTimeline();
      else if (state.view === "tree") applyTreeSearch();
      if (state.view === "browse") history.replaceState(encodeState(), "", "?" + stateToQuery(encodeState()));
    });
    $("#globalSearch").addEventListener("keydown", e => {
      if (e.key === "Enter" && state.search.trim()) {
        if (state.view !== "browse" && state.view !== "timeline") switchView("browse");
        else if (state.view === "tree") applyTreeSearch();
      }
    });
    ...
    // 后退 / 前进按钮
    const back = $("#hist-back"), fwd = $("#hist-fwd");
    if (back) back.addEventListener("click", () => { if (window.__canBack) history.back(); });
    if (fwd) fwd.addEventListener("click", () => { if (window.__canFwd) history.forward(); });

    // popstate：浏览器前进 / 后退时恢复状态，并维护栈深度标记
    window.addEventListener("popstate", e => {
      const st = e.state ? decodeState(e.state) : queryToState(location.search);
      restoring = true;
      applyState(st);
      restoring = false;
      // 根据浏览器前进/后退方向调整深度标记
      ...
      window.__canBack = histDepth > 0;
      // 前进：若之后还有本站 push 过的历史则仍可能前进（简化：前进后清空前进可能性，靠浏览器原生前进）
      window.__canFwd = false;
      updateHistButtons();
    });

    // 首屏：按 URL 解析初始状态（支持带参直达链接）
    const initState = queryToState(location.search);
    restoring = true;
    applyState(initState);
    restoring = false;
    // 用 replaceState 写入首屏状态，使后续后退可回到它（不增加深度）
    history.replaceState(encodeState(), "", "?" + stateToQuery(encodeState()));
    histDepth = 0;
    refreshHistFlags();

    $("#footer-count").textContent = `收录 ${TECHS.length} 项技术 · ${CATEGORIES.length} 分类 · ${ERAS.length} 时期`;
  }
