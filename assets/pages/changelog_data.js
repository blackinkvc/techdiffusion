...
,
    ...
    },
    {
        ...
        "date": "2026-09-15",
        "type": "docs",
        "title": "登记 · 阶段 3 草案入台账并同步纠错页与版本日志：E 族 info 前置逐条判定 3.1c（18 条）+ 3.1d（16 条），均待审核未写盘",
        "summary": "本条**不改变任何数据** —— 主管线仍为节点 2,266 / 依赖边 6,167，全量管线 `graph.json`（2,299 / 6,963）与 `assets/data_full.js`（13,318 / 16,811）未变。本轮落地的是两份**判定草案**与配套登记：沿用 3.1a / 3.1b 的「先出草案入台账 → 用户逐条审核 → 执行器写盘」流程，把 E 族 info 91 条中的两簇逐条判定写入 `audit/ledger.json` 的 `batches[\"3.1c\"]` 与 `batches[\"3.1d\"]`，并同步《技术网络检修计划.md》、纠错页与本页 / 《版本迭代日志.md》。判定结果：**3.1c** 18 条（通信、网络与标识设备）保留 0 / 改前置 1 / 删边 17 / 补前置 17 条共 37 条边，预演依赖边 6,167 → **6,187**；**3.1d** 16 条（信息记录、存储与音视频设备）保留 0 / 改前置 2 / 删边 14 / 补前置 16 条共 30 条边，预演依赖边 6,167 → **6,183**。两批节点集合不相交，写盘顺序不限。",
        "treeChange": {
            "scope": "**无（本条为登记版本，不改动任何数据）**：主管线节点 2,266 与依赖边 6,167 均不变；全量管线 `graph.json`（2,299 / 6,963）与 `assets/data_full.js`（13,318 / 16,811）未变；`assets/techs_*.js`、`assets/techs_midtech.js` 与 `audit/baseline.json` **均未改动**。改动落在登记层：`audit/ledger.json` 新增 `batches[\"3.1c\"]` 与 `batches[\"3.1d\"]`；《技术网络检修计划.md》批次表两行 + 当前状态 + §六 + 审核节点段；纠错页两条 CR 条目与待审核队列 2 条；本页与《版本迭代日志.md》。",
            "reason": "阶段 3 的语义判定批采用「先出草案入台账 → 用户逐条审核 → 执行器写盘」的流程（自 3.1a 定型）。草案是把「每条边该保留 / 改前置 / 删边」的判定与理由固化下来供逐条复核的产物，属需要留痕的项目状态；3.1a / 3.1b 的草案当时只进了台账与检修计划，本轮补齐纠错页与版本日志两处同步，使「当前有哪些待审判定、预演规模如何、改动哪些边」在站点上可直接查阅，不必翻台账 JSON。",
            "detail": "① **候选与硬断言**：3.1c 断言 `category === \"info\"` 且「单前置且前置 = `mathematics`」命中 18 条；3.1d 同断言命中 16 条；两批合计覆盖 E 族 info 91 条中的 34 条。② **判定 3.1c**：保留 0 / 改前置 1（`inf_antenna` 天线 → `em_theory` 电磁理论）/ 删边 17 / 降级 0。③ **判定 3.1d**：保留 0 / 改前置 2（`inf_vinyl` 黑胶唱片 → `acoustics` 声学、`inf_dataviz` 数据可视化 → `statistics` 统计学）/ 删边 14 / 降级 0。④ **repoint 判据（3.1c 新立、3.1d 沿用）**：定义性构成本身即学理者才改前置（天线＝电磁理论的物化、黑胶唱片＝声—机械振动转换、数据可视化＝图形编码统计关系）；定义性构成是器件 / 材料 / 网络机制者走删边 + 补前置。⑤ **补前置**：3.1c 17 条目共 37 条边、3.1d 16 条目共 30 条边，全部取库内既有节点且前置年 ≤ 本节点年。⑥ **核账**：新增边 = Σ`backfills.add` + Σ(repoint 各 1 条 `newParent`)。⑦ **预演校验**：前置存在性 0 错、年份倒挂 0、无环、零前置维持 4；两批均通过执行器干跑（完整划分断言 / 时序性 / 环），并打印逐节点变更明细。⑧ **未改动**：全部数据文件、`baseline.json`、`theory_data.js`；两批共 34 条节点的判定均待用户逐条审核后才由 `tools/apply_ledger.js --batch <批> --apply` 写盘。"
        },
        "changes": [
            "① 本条性质：**登记版本，零数据改动**。主管线节点 2,266 / 依赖边 6,167 不变（与 v0.9.30 一致），全量管线与基线文件均未触碰；只新增台账草案与四处登记文本。",
            "② 批次 3.1c 草案（E 族 info · 通信、网络与标识设备 18 条）：候选硬断言命中 18 条，判定 **保留 0 / 改前置 1 / 删边 17 / 降级 0**，`backfills` 17 条目共 37 条边。改前置仅 1 条 —— `inf_antenna` 天线 → `em_theory` 电磁理论（Hertz 1888 以天线实验验证电磁波，学理即其定义性构成）。删边 17 条全部补入域内真实前置，如 `inf_fiber` 光纤通信 → `optic_fiber` + `lasers`、`inf_router` 路由器 → `packet_switch` + `computer`、`inf_5g` → `radio` + `inf_phasedarray` + `inf_sdn`、`inf_barcode` 条形码 → `printing` + `electronics`。",
            "③ 批次 3.1d 草案（E 族 info · 信息记录、存储与音视频设备 16 条）：候选硬断言命中 16 条，判定 **保留 0 / 改前置 2 / 删边 14 / 降级 0**，`backfills` 16 条目共 30 条边。改前置 2 条 —— `inf_vinyl` 黑胶唱片 → `acoustics` 声学（声波—机械振动转换是其定义性构成）、`inf_dataviz` 数据可视化 → `statistics` 统计学（与 3.1b 的 `mfg_sixsigma` → `statistics` 同一判据）。删边 14 条补入介质 / 器件 / 加工母体，如 `inf_tape` 磁带 → `plastics` + `electromagnetism`、`inf_hdd` 硬盘 → `inf_tape` + `mfg_precisionbearing`、`inf_flash` 闪存 → `semiconductor` + `ic`。",
            "④ 两批**保留数均为 0**，与 3.1b（41 条保留 0）构成同一结论：E 族的定义（单前置且该前置属 14 个泛化枢纽）本身即是「批量赋值为学科级顶点」的判别式，该槽位不区分必要与非必要；**四标准中的排他性是筛出这类挂载的唯一判据**——时序性对其整体通过（学科级节点总是更早），无任何判别力。",
            "⑤ 核账与预演：新增边一律按「Σ `backfills.add` + Σ(repoint 各 1 条 `newParent`)」计 —— 3.1c 为 37 + 1 = **38** 条（移除 18，净 +20，6,167 → **6,187**）；3.1d 为 30 + 2 = **32** 条（移除 16，净 +16，6,167 → **6,183**）。`mathematics` 作为前置的节点：3.1c 后 570 → 552、3.1d 后 570 → 554（两批基数相同，均在当前盘面上预演；若连续写盘则分别为 6,187 与 6,203、数学前置 552 与 536）。",
            "⑥ 预演校验方式：两批各写独立的构建 + 校验脚本（候选硬断言 → 前置存在性与年份校验 → 预演新边集后查环 / 倒挂 / 零前置 → 边数核账），全部通过；随后用正式执行器 `tools/apply_ledger.js --batch 3.1c|3.1d`（不带 `--apply`）干跑复核，两道断言（完整划分、时序性）与环校验均通过，并打印逐节点「旧前置 → 新前置」明细。",
            "⑦ 登记内容：`audit/ledger.json` 新增 `batches[\"3.1c\"]`（键序插于 `3.1b` 与 `5.0` 之间）与 `batches[\"3.1d\"]`（插于 `3.1c` 与 `5.0` 之间），二者结构与 3.1a / 3.1b 完全一致（`edges` 逐边 verdict/newParent/reason + `backfills` child/add/reason + `summary` + `verification`）；《技术网络检修计划.md》批次表新增两行、3a 行更新、§六「下一批」段、审核节点段、当前状态行共 9 处；纠错页新增 CR-2026-0915-einfo-math-hub-3.1c 与 CR-2026-0915-einfo-math-hub-3.1d，待审核队列 26 → 28 条；本页与《版本迭代日志.md》各一条。",
            "⑧ 判定取证的一条教训（已写入台账 `findings`）：**节点自身的 `summary` / `significance` / `views[].text` 是派生物、不是独立证据**。这两批 34 条节点的摘要全部由旧前置（`mathematics`）模板生成（如「通信电缆在 1850 年 前后成形，依托 数学 把 信息记录、远程传输 落到实处」），照抄会把错误前置当成判定依据。本批判定一律以领域事实 + 核查库内**实际可用**的节点 id 与年份为准（先把全库导出为 `年份 / 分类 / id / 名称` 索引再检索候选）。",
            "⑨ 判定中新发现的缺口（补节点候选累计 9 项）：前两批新增 5 项 —— **光交叉连接 / 光交换设备**（全光网络的构成之一）、**蜂窝 / 移动通信世代节点 1G–4G**（5G 的直接前身）、**色料 / 染料**（复写纸的转移涂层）、**磁记录介质 / 磁头**（磁带与硬盘共用）、**图像 / 信号处理**（数字水印的嵌入与提取侧）。这些缺口已在批次草案中按「域内可考代位 + 明确标注」处理，另立批次处置。",
            "⑩ 未改动与下一步：全部数据文件（`techs_extra.js` / `techs_extend.js` / `techs_more.js` / `techs_midtech.js` / `data.js`）、`audit/baseline.json`、`theory_data.js` 覆盖层与全量管线两文件均未改动，三道门禁在本条提交时全绿。下一步为**用户逐条审核**这两批草案（审核要点已在纠错页待审队列逐条列出）；审核通过后按 3.1c → 3.1d 或反序执行 `apply_ledger --apply`，并走派生链重算（`regen_text` → `regen_midtech`，各自复跑验不动点）→ 基线刷新 → 三处登记。**根治手段仍在阶段 5c**（生成器槽位赋值的语义硬约束），本两批属事后清理。"
        ],
        "files": [
            "audit/ledger.json",
            "技术网络检修计划.md",
    ...
    },
    {
        ...
        "changes": [
            ...
            "④ **全库 2,267 条完成机械打标**：主维度求解路径 prefix 1,838 / cat 255 / prefix:bx 160 / hub-head 14；任务域路径 p2 826 / kw 809 / p1 432 / catSub 200。门类分布 eng 857（37.8%）、life 339、info 283、matter 250、mil 237、math 126、phys 90、symbol 69、chem 10、earth 6。**分布偏斜与 2 个零命中任务域（`energy/animal_human`、`manufact/food_proc`）是机械兜底的已知局限**，由 D1 例外表逐条修正。",
    ...
    },
    {
      ...
      "date": "2026-09-15",
      "type": "docs",
      "title": "登记 · 批次 D1「维度层例外表」草案入台账（未写盘）：拟 overrides 50 条修正 `basic` 类学理节点的门类落空；澄清 `eng` 37.8% 非打标错误；给出 2 个零命中任务域的可达性结论",
      "summary": "**本条不改动任何数据** —— 主管线仍为 2,267 个节点 / 6,169 条依赖边；`assets/techs_*.js`、`assets/techs_midtech.js`、`audit/baseline.json` 均未触碰；全量管线 `graph.json`（2,299 / 6,963）与 `assets/data_full.js`（13,318 / 16,811）未动。改动落在登记层：`audit/ledger.json` 新增批次 `D1`（`status` = `draft-ready`），并同步纠错页、版本日志与《技术网络检修计划》。**`assets/pages/dimension_data.js` 的 `overrides` 在本草案获批前保持为空。**",
      "changes": [
        "① 本条性质：**D0 的后续草案轮，零数据改动**。D0 已声明「门类分布偏斜与 2 个零命中任务域是机械兜底的已知局限，不得据此认为打标已经准确」，D1 即逐条处理这些局限，产出例外表草案供审核。",
        "② **缺陷 A（主要）**：`category=basic` 共 302 条（全库学理级节点），其中 **68 条**的 `field.primary` 落空为 `symbol`。根因是 primary 兜底末级只有一张 `category → 门类` 表，而 `basic` 是**跨学科**分类（数学 / 物理 / 化学 / 天文 / 生物 / 方法论同处一类），任何单一映射都不成立，`symbol` 成为落空值的实际承担者。",
        "③ **关键取证：`sub` 轴并未失效** —— 同一批 68 条的 `sub` 多已被 taxonomy 关键词判到具体学科子域（`basic/math` / `basic/physics` / `basic/chemistry` / `basic/astronomy` / `basic/earthsci` / `basic/biosci`），是 `primary` 没有利用 `sub` 已有的信息。故修法为**读 `sub` 定 `primary`**，而非新增规则。",
        "④ 拟 `overrides` **50 条**：**A 组 30 条** —— `sub` 已判到具体学科子域，门类随之确定（`astronomy`→earth、`thermodynamics`→phys、`valence`→chem、`evolution`→life、`algebra`→math、`cartography`→earth 等）；**B 组 20 条** —— `basic/method` 无学科指向，按节点语义逐条判定（`complexity`→info、`black_hole`→earth、`neuroscience`→life、`fut_ai_scientist`→info 等）。",
        "⑤ 建议保留 `symbol` **6 条**：`language` / `counting` / `positional` / `decimal` / `gesture_comm` / `symbolic_art` —— 符号系统本体，D0 的兜底结论本就正确。",
        "⑥ **需用户裁定 12 条**（草案附我的建议）：`scientific_method` / `peer_review` / `replication`（方法论 / 学术制度，无对应门类）、`metrology` / `surveying`（→eng）、`microscope`（→life）、`nanoscience`（→matter）、`cognition`（→life）、`fire_making`（→eng）、`magnet`（→matter）、`black_hole_comp`（→info）、`fut_spacetime_engineering`（→phys）。",
        "⑦ **澄清：`eng` 857（37.8%）不是打标错误** —— 实测它由 `category → 门类` 映射决定：`transport` 244 + `build` 242 + `energy` 215 + `manufact` 153 + 3 = 857。只要「能源 / 运输 / 建筑 / 制造」四类都归入「工程与建造」，该比例就与兜底规则无关；要改变它必须**拆分门类**（如单设「能源」「运输」），属维度层设计变更，不属例外表。故本草案不动 `eng`。",
        "⑧ **零命中任务域结论**：`manufact/food_proc` 可由 `watermill`（水磨，现误挂 `manufact/cutting`）填补 1 条；`energy/animal_human`（畜力与人力）在当前分类下**不可达** —— 全库唯一畜力节点 `animal_draft` 的 `category` 是 `transport`，与 `energy/*` 不同类，受「`sub` 须与 `category` 同类」硬约束阻断。草案给出两条出路：A 保持零命中并登记为「已知不可达」（默认，不改 category）；B 同批把 `animal_draft` 的 category 改为 energy（属改动节点自带字段，需单独裁定）。",
        "⑨ 同批列出 **4 条 `sub` 明显误挂**：`watermill`（`manufact/cutting`→`manufact/food_proc`，水磨是谷物研磨非切削）、`animal_draft`（`transport/water`→`transport/road`）、`ene_tidal`（`energy/fossil`→`energy/geo_ocean`）、`mat_wave`（`energy/power_machine`→`energy/geo_ocean`，该域定义即「地热与海洋能」）。",
        "⑩ 门类分布预演（合计 2,267 不变）：`symbol` 69 → **19**、`earth` 6 → **15**、`chem` 10 → **15**、`math` 126 → 143、`phys` 90 → 100、`life` 339 → 346、`info` 283 → 285；**`eng` 857 / `matter` 250 / `mil` 237 不变**。两条最小门类获得主要修正。",
        "⑪ **本批未做的事**：未改动 `assets/pages/dimension_data.js`（`overrides` 保持为空）；未改动任何 `dependsOn`（属 D2 / D3）；未拆分 `eng` 门类；未复核 160 条 `bx_*` 占位节点（属另立批次）；未做全量 `sub` 复审。"
      ],
      "files": [
        "audit/ledger.json（新增批次 D1，status = draft-ready；批次键 19 → 20）",
        "/tmp/D1_例外表草案.md（草案全文：逐条明细表 + 预演）",
        "assets/pages/correction_data.js（新增 CR-2026-0915-dim-basic-fallback-symbol，entries 24 → 25；pendingQueue 30 → 31）",
        "assets/pages/changelog_data.js（本条目，entries 38 → 39）",
        "版本迭代日志.md（表格行 + 详细章节）",
