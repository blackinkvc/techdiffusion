...
window.CORRECTIONS = {
  meta: {
    ...
    pendingQueue: [
      ...
      "【v0.9.20 · 已执行 · 生成式依赖网全量重建】位置式模板伪边三族已全部过账：① 严格「时间最近」变体 778 → 保留 106 / 删 672；② 同年份变体 287 → 保留 29 / 删 258；③ 跨分类同年「前沿并列」变体 97 → 保留 17 / 删 80；合计处置 1,142 条、删 1,015 条、97 个节点补写真实前置。依赖边 7,328 → 6,549；位置式边 806 → 152；环 0 / 年份倒挂 0 / 悬空 0。**仍未做**：ⓐ 重写 1027 条模板文案（本轮只改边、未改文本，故部分节点简介仍写着已删掉的旧前置，如 inf_pencil 简介仍称「依托 锆、蒸汽机、数学」）——这是下一批的首要任务，口径为「按新 dependsOn 重写、不绑定具体边、不得编造」；ⓑ 给 tools/gen_techs.js · gen_run.js · upgrade_stubs.js 加语义硬约束，防止再生（当前仅手工清理，管线未加锁，重跑工具会复现）；ⓒ 其余 22 组同名节点去重。**口径**：年份仅作参考，以本质与逻辑为准。"
    ],
    enablesAudit: {
  ...
  entries: [
    {
      ...
      category: "meta",
      severity: "高",
      status: "已修正（全量）· v0.9.20：三族位置式伪边 1,142 条逐条判定，删 1,015 条、保留核验通过 152 条、97 个节点补写真实前置；余下 1027 条模板文案待另批",
