...
(function () {
  ...
  <div style="border:1px solid #e0a800;background:#fff8e1;border-radius:10px;padding:12px 16px;margin:14px 0;font-size:14px;color:#5d4a00">
    <b>⚠ Phase 3 · 进阶模型可运行化 —— 方案已设计并储存，暂待硬件执行</b><br>
    当前开发环境<b>无 GPU / TPU 硬件</b>，故 M1–M10 的脚本执行与算力实测<b>暂停</b>（已设计内容见下方折叠详情，不会丢失）。硬件就绪后（本地 CUDA / Colab / 云 T4·A100 / TPU），按第九节各卡片的「后续实现」路径运行即可；运行结果须与本页实时统计（滞后分布 / 共生指数 / 重要节点）交叉校验。
  </div>
  <details style="border:1px solid #d8d0c0;border-radius:8px;padding:8px 12px;margin:0 0 14px;background:var(--accent-bg);font-size:13.5px">
    <summary style="cursor:pointer;font-weight:600">▸ 已储存的 Phase 3 方案（待硬件 · 展开查看）</summary>
    <div style="margin-top:8px;line-height:1.75">
      ① <b>最小可运行脚本（设计定稿）</b>：M1 = node2vec / gensim 在 2289 节点做随机游走 + skip-gram（CPU 数分钟出向量 Z）；M2 = PyTorch Geometric 两层 GCN，节点分类头预测类别 / 年代档 / 可行性档；M4 = PyG GAEn（内积解码）做缺失依赖边链路预测，高置信预测边经本页 lift + 滞后二次校验后列为回写候选。<br>
      ② <b>算力成本量级（设计估算，实测后回填）</b>：M1 ≈ 0.01 GPU·h 当量（CPU 可跑）；M2/M4 单卡 GPU 约 1–4 GPU·h（云 T4 ≈ $0.4/h 牌价量级）；M3/M5/M6/M9 按数据规模 5–40 GPU·h；M7 全量生成建议 TPU 集群（TPU v4 ≈ $3.2/芯片·h 牌价量级）；M8 MCMC CPU 小时级。上全量 13k 语料各档 ×3–10。<br>
      ③ <b>第九节速查表将补「数据需求量」列</b>：小=2289 精编集已够 / 中=建议 13k 全量 / 大=需全量 + 外部史料语料。<br>
      ④ <b>验收协议</b>：跑通后把嵌入 Z / 预测置信与本节滞后分布、9×9 共生热力图、样本外 MAE/RMSE 交叉核对，一致才算通过；结果回填本详情与建模方法日志。
    </div>
  </details>
